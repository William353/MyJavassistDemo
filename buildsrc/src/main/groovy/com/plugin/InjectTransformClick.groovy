...
class InjectTransformClick extends Transform {

    Project mProject

    public InjectTransformClick(Project project) {
        mProject = project
    }

    // Transfrom在Task列表中的名字
    // transformClassesWithInjectClickForDebug
    @Override
    String getName() {
        return "InjectClick"
    }
    ...
    void transform(Context context, Collection<TransformInput> inputs, Collection<TransformInput> referencedInputs,
        ...
        inputs.each { TransformInput input ->
            input.directoryInputs.each { DirectoryInput directoryInput ->
                ...
                InjectClickUtil.injectDir(directoryInput.file.getAbsolutePath(), packageName.replace('.','/'), mProject)
